...
<div class='bg-dark col-lg-6 p-0 min-vh-100 col-md-12'>
    ...
    <div class='px-4 pt-5 pb-5 mb-5'>
        <div class="card border-secondary text-white mt-5 bg-transparent">
            <div class="card-body">
                <h3 class="card-title m-0 p-0 text-center">{{ anime }}</h3>
            </div>
        </div>
        ...
        <div v-for='k in data.result' class='mt-5'>
            <div  :id='k.id' class="card border-secondary rounded-0 rounded-top text-white pt-5 p-3 bg-transparent">
                <div class="card-body">
                    ...
                    <p class='text-center mb-0 pb-0 mt-3'><small><i>~ {{ k.character }} ~</i></small></p>
                </div>
            </div>
...
export default {
    ...
    async asyncData({
        ...
        let anime = params.judul;
        let page = 1;
        let data = await $axios.$get(`/api/getbyanime?anime=${anime}&page=${page}`)
        let bahasa = "";
        return {
            anime,
            data,
    ...
    methods: {
        ...
        infiniteHandler($status) {
            this.page++;
            this.$axios.$get(`/api/getbyanime?anime=${this.anime}&page=${this.page}`)
                .then(response => {
                    if (!response.next) {
                        $status.complete();
                    }
